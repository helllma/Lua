...
extern int creer_socket(int type, int *ptr_port, struct sockaddr_in *ptr_adresse);
...
//   syslog(LOG_ERR,commentaire);
   ...
   tp->th_msg[length] = '\0';
   length += 5;
   if (sendto(peer, buf, length, 0,(struct sockaddr*)to,tolen) != length) 
...
//	            syslog(LOG_ERR,"erreur select.\n");
      ...
      if ((size<vPKTSIZE)&&(nextBlockNumber!=0)) break; // tout a déj?ét?envoy?dans le 1er packet de DATA
